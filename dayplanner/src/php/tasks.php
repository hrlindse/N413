...
if (isset($_GET['id']) && $_SERVER['REQUEST_METHOD'] === 'GET'){
    ...
    $sql = "SELECT 1 FROM `tasks` WHERE id = ". $_GET['id'] ;
    ...
    $sql = "SELECT id, userID, title, date, completed, description, priority, projectID, tags FROM `tasks`";
    $result = mysqli_query($link, $sql) or trigger_error("Query Failed! SQL: $sql - Error: ".mysqli_error($link), E_USER_ERROR);
    $records = array();
    while ($row = mysqli_fetch_array($result, MYSQLI_BOTH)){
        $records[] = $row;
...
//    if($result = "") {
        //post
        $sql = "INSERT INTO `tasks` (userID, title, description, date, projectID, tags, priority) VALUES ( 1, '" . $title .
